...
echo Port: 8000
echo Parallel Slots: 5 (Workers:2 + Memory:1 + Eval:1 + TaskGen:1)
...
powershell -Command "& { .\llama-server.exe --model '..\models\LFM2-1.2B-F16.gguf' --port 8000 --host 0.0.0.0 --n-gpu-layers 999 --ctx-size 32768 --parallel 5 --cont-batching 2>&1 | Tee-Object -FilePath '..\logs\llm_server.log' }"
